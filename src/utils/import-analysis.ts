import { isJavaScriptOrTypeScript } from "./language-utils";
...
function findJsTsImportSpans(lines: string[], spans: LineSpan[]): void {
  ...
  while (i < lines.length) {
    ...
    if (
      ...
      else if (!trimmedLine.endsWith(";") && !trimmedLine.includes(" from ")) {
    ...
    // and stop scanning for imports once you've clearly left the import block
    if (
      i > 0 &&
      spans.length > 0 &&
      trimmedLine !== "" &&
      ...
      // Buffer Check: if we're still in the import region (allow blank/comment lines),
